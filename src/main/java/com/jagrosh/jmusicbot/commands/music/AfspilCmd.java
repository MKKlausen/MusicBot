 ...
 */
public class AfspilCmd extends MusicCommand
{
    ...
    public void doCommand(CommandEvent event) 
    {
        if(event.getArgs().isEmpty() && event.getMessage().getAttachments().isEmpty())
        {
            ...
            Set<String> playlistOptions = playlists.keySet();
            
            playlistOptions.forEach(p -> {
                builder.append("\n`").append(event.getClient().getPrefix()).append(name).append(" " + p + "`");
